...
bool CelestialBody::Initialize()
{
   ...
   if (userDefined)
   {
      ...
      {
         if (rotationSrc != Gmat::SPICE_KERNEL)
         {
...
//   }
   ...
   if (id == ROTATION_DATA_SRC)
   {
      ...
      if (value == "SPICE")
      {
         for (int i = 0; i < GmatSolarSystemDefaults::PLANET_NAMES->size(); i++)
         {
            if (GetName() == GmatSolarSystemDefaults::PLANET_NAMES[i] || GetName() == GmatSolarSystemDefaults::SUN_NAME)
            {
               throw SolarSystemException("Error: Setting the RotationDataSource to \"SPICE\" on a default planet or the Sun is not allowed. Found body: " + GetName() + "\n");
            }
         }
         SetRotationDataSource(Gmat::SPICE_KERNEL);
      }
      else
      {
...
//      }
   ...
   {
      //This parameter is also handled in SpacePoint but we catch it here to handle more functionality
      spiceFrameID = value;
      return true;
   }
...
//---------------------------------------------------------------------------
//  SpiceOrbitKernelReader* GetSpiceKernelReader()
//---------------------------------------------------------------------------
/**
 * Returns the Spice Kernel reader
 *
 */
 //---------------------------------------------------------------------------
SpiceOrbitKernelReader* CelestialBody::GetSpiceKernelReader()
{
   return kernelReader;
}
...
//------------------------------------------------------------------------------
/**
 * This method returns a flag indicating whether or not Spice and the SpiceKernelReader
 * has been set up yet or not. If not SetUpSpice() can be called.
 *
 * @return true if Spice is set up, false if not.
 *
 */
 //------------------------------------------------------------------------------
bool CelestialBody::IsSpiceSetUp()
{
   return spiceSetupDone;
}
...
Gmat::RotationDataSource CelestialBody::GetDefaultRotationSource()
{
   return default_rotationSrc;
}
