...
//   Rvector3 p1 = stationLoc;
   ...
   p2 = p1 + dR;
   electdensity = elctronDensities[0];                // unit: electron / m^3
   ...
   {
      p2 = p1 + dR;
      electdensity = (elctronDensities[i] + elctronDensities[i-1]) / 2.0;                // unit: electron / m^3
      ds = (p2-p1).GetMagnitude()*GmatMathConstants::KM_TO_M;   // unit: m
...
Real Ionosphere::BendingAngle(Rvector3 end, Rvector3 start)
{
   ...
   density_i1 = elctronDensities[NUM_OF_INTERVALS-1];
...
// Solution to where a line intersects a sphere is a quadratic equation
   ...
   if (discriminant <= 0)
   {
      calcVal = false; // Path does not travel through ionosphere
   }

   Real d1, d2; // Roots of quadratic equation
   Real val = GmatMathUtil::Sqrt(b*b - 4.0*a*c);
   d1 = (-b - val) / (2.0*a);
   d2 = (-b + val) / (2.0*a);

   if ((d1 > 1 && d2 > 1) || (d1 < 0 && d2 < 0))
   {
      calcVal = false; // Segment between start and end does not travel through ionosphere
   }
   ...
   elctronDensities = {};
   GetElectronDensities(end, start);
   ...
   if(calcVal)
      tec = TEC(end, start);                  // Equation 6.70 of MONTENBRUCK and GILL      // unit: number of electrons/ m^2
   ...
   if (calcVal)
      dphi = BendingAngle(end, start);                                                      // unit: radian
...
void Ionosphere::GetElectronDensities(Rvector3 end, Rvector3 start)
{
   Rvector3 dR = (end - start) / NUM_OF_INTERVALS;
   Rvector3 p1 = start;
   Rvector3 p2;
   ...
   for (int i = 0; i < NUM_OF_INTERVALS; ++i)
   {
      ...
      electdensity = ElectronDensity(p2);                // unit: electron / m^3
      elctronDensities.push_back(electdensity);
