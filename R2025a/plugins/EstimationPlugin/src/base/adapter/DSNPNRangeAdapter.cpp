...
//------------------------------------------------------------------------------
const std::string
DSNPNRangeAdapter::PARAMETER_TEXT[DSNPNRangeAdapterParamCount - RangeAdapterKmParamCount] =
{
   "RangeModuloConstant",
};
...
DSNPNRangeAdapter::PARAMETER_TYPE[DSNPNRangeAdapterParamCount - RangeAdapterKmParamCount] =
{
   Gmat::REAL_TYPE,
};
 ...
 */
//------------------------------------------------------------------------------
DSNPNRangeAdapter::DSNPNRangeAdapter(const std::string& name) :
   RangeAdapterKm      (name),
   rangeModulo         (1.0e18)
{
   typeName = "DSN_PNRange";              // change type name from "RangeKm" to "DSN_PNRange"
   parameterCount = DSNPNRangeAdapterParamCount;
 ...
 */
//------------------------------------------------------------------------------
DSNPNRangeAdapter::~DSNPNRangeAdapter()
{
}


//------------------------------------------------------------------------------
// DSNPNRangeAdapter(const DSNPNRangeAdapter& dsnr)
//------------------------------------------------------------------------------
/**
 * Copy constructor
 *
 ...
 */
//------------------------------------------------------------------------------
DSNPNRangeAdapter::DSNPNRangeAdapter(const DSNPNRangeAdapter& dsnr) :
   RangeAdapterKm      (dsnr),
...
//------------------------------------------------------------------------------
// DSNPNRangeAdapter& operator=(const DSNPNRangeAdapter& dsnr)
//------------------------------------------------------------------------------
/**
 * Assignment operator
 *
 ...
 *
 * @return This adapter made to look like dsnr
 */
//------------------------------------------------------------------------------
DSNPNRangeAdapter& DSNPNRangeAdapter::operator=(const DSNPNRangeAdapter& dsnr)
{
   if (this != &dsnr)
   {
 ...
 */
//------------------------------------------------------------------------------
GmatBase* DSNPNRangeAdapter::Clone() const
{
   return new DSNPNRangeAdapter(*this);
}
 ...
 */
//------------------------------------------------------------------------------
std::string DSNPNRangeAdapter::GetParameterText(const Integer id) const
{
   if (id >= RangeAdapterKmParamCount && id < DSNPNRangeAdapterParamCount)
 ...
 */
//------------------------------------------------------------------------------
Integer DSNPNRangeAdapter::GetParameterID(const std::string& str) const
{
   for (Integer i = RangeAdapterKmParamCount; i < DSNPNRangeAdapterParamCount; i++)
   {
 ...
 */
//------------------------------------------------------------------------------
Gmat::ParameterType DSNPNRangeAdapter::GetParameterType(const Integer id) const
{
   if (id >= RangeAdapterKmParamCount && id < DSNPNRangeAdapterParamCount)
 ...
 */
//------------------------------------------------------------------------------
std::string DSNPNRangeAdapter::GetParameterTypeString(const Integer id) const
{
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::GetRealParameter(const Integer id) const
{
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::SetRealParameter(const Integer id, const Real value)
{
   if (id == RANGE_MODULO_CONSTANT)
   {
      if (value <= 0.0)
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::GetRealParameter(const std::string &label) const
{
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::SetRealParameter(const std::string &label, const Real value)
{
 ...
 */
//------------------------------------------------------------------------------
bool DSNPNRangeAdapter::RenameRefObject(const UnsignedInt type,
 ...
 */
//------------------------------------------------------------------------------
bool DSNPNRangeAdapter::Initialize()
{
   bool retval = false;
 ...
 */
//------------------------------------------------------------------------------
const MeasurementData& DSNPNRangeAdapter::CalculateMeasurement(bool withEvents,
      ObservationData* forObservation, std::vector<RampTableData>* rampTable, 
      bool forSimulation)
{
   #ifdef DEBUG_RANGE_CALCULATION
      MessageInterface::ShowMessage("DSNPNRangeAdapter::CalculateMeasurement(%s, "
   ...
   if (rampTable != NULL)
   {
       ...
       catch (MeasurementException exp)
       {
           ...
           //Do not throw an error if the Ramp table exists but 
           //no ramp record is found for a given time or gs
   ...
   //Transmit and receive delays from GMD file
   Real receiveDelay = 0.0;
   Real transmitDelay = 0.0;

   obsData = forObservation;
   //Reset value for range modulo constant
   if (obsData)
   {
       rangeModulo = obsData->rangeModulo;                                                                         // unit: RU
       receiveDelay = obsData->receiveDelay;
       transmitDelay = obsData->transmitDelay;

       //Adjust the time tag to account for receive delay (t3e->t3r), noted in PN Range spec 2.1
       obsData->epochGT.SubtractSeconds(receiveDelay);
       obsData->epoch = obsData->epochGT.GetMjd();

       //Compute range in km
       RangeAdapterKm::CalculateMeasurement(withEvents, obsData, rampTB, forSimulation);
   }
   else
       //Compute range in km
   ...
   if (obsData)
   {
       //Add in receive delay back to the time before integration
       cMeasurement.epochGT.AddSeconds(receiveDelay);
       cMeasurement.epoch = cMeasurement.epochGT.GetMjd();

       obsData->epochGT.AddSeconds(receiveDelay);
       obsData->epoch = obsData->epochGT.GetMjd();
   }
   //Convert range from km to RU and store in cMeasurement:
   for (UnsignedInt i = 0; i < cMeasurement.value.size(); ++i)
   {
      //Specify uplink frequency
      ...
      if (obsData)
      {
          //This checks to ensure all bands are the same, the band we actually use, and the bands specified by the observation
          if (freqBand != obsData->uplinkBand || freqBand != obsData->downlinkBand)
          {
              std::stringstream ss;
              Real ep = theTimeConverter->ConvertToTaiMjd(obsData->epochSystem, obsData->epochGT.GetMjd(),
                  GmatTimeConstants::JD_NOV_17_1858);
              char epochbuffer[200];
              sprintf(epochbuffer, "%18.12lf", ep);
              ss << "Error: The frequency bands for the observation at epoch: " << epochbuffer << " do not match.\n Uplink or downlink frequency band mismatch  ";
              ss << "in the GMD file or ramp table (if used). GMAT does not currently support cross-band tracking measurements.\n\n";
              ss << "Bands found:\n    Uplink:     " << obsData->uplinkBand << "\n    Downlink:   " << obsData->downlinkBand << "\n    Band of frequency: " << freqBand << "\n";
              throw MeasurementException(ss.str());
          }
      }
      // Specify multiplier (at time t1)
      // multiplier only changes its value only after range in km is computed
      multiplier = GetFrequencyFactor(uplinkFreq*1.0e6);

      // Convert correction from km to RU. 
      ...
      //Convert range from km to RU in cMeasurement.value
      Integer errnum = 0;
      
      //this travel time, comparing to the PNRange spec, is t1t->t3r. We adjust to t1e->t3e below
      Real realTravelTime = cMeasurement.value[i]/lightspeed;                                                     // unit: seconds
      
      //adjust to t1e->t3e by adding in the delays to increase the total travel time
      realTravelTime = realTravelTime + receiveDelay + transmitDelay;

      if (rampTB != NULL)
      {
         // ramped frequency
         #ifdef DEBUG_RANGE_CALCULATION
            MessageInterface::ShowMessage("Calculate DSN_PNRange based on ramp table\n");
         #endif
         ...
         {
            cMeasurement.value[i] = 0.0;               // No C-value due to the failure of the calculation of IntegralRampedFrequency()
            cMeasurement.isFeasible = false;
            cMeasurement.unfeasibleReason = "R";
      ...
      else
      {
         // constant frequency
         #ifdef DEBUG_RANGE_CALCULATION
            MessageInterface::ShowMessage("Calculate DSN_PNRange based on constant frequency\n");
         #endif
      ...
      if (measurementType == "DSN_PNRange")
      {
         // Compute bias
         ComputeMeasurementBias("Bias", "DSN_PNRange", 2);
         // Compute noise sigma
         ComputeMeasurementNoiseSigma("NoiseSigma", "DSN_PNRange", 2);
      ...
      //Is reset below if obsData exists
      cMeasurement.downlinkBand = freqBand;
      cMeasurement.rangeModulo = rangeModulo;

      if (obsData)
      {
          cMeasurement.downlinkBand  = obsData->downlinkBand;
          cMeasurement.transmitDelay = obsData->transmitDelay;
          cMeasurement.transmitDelay = obsData->receiveDelay;
      }
      ...
      cMeasurement.tropoCorrectValue = multiplier*GetTropoCorrection()/lightspeed;                      // convert Km to RU
         ...
         MessageInterface::ShowMessage("====  DSNPNRangeAdapter: Range Calculation for Measurement Data %dth  \n", i);
         MessageInterface::ShowMessage("===================================================================\n");
 ...
 */
//------------------------------------------------------------------------------
const std::vector<RealArray>& DSNPNRangeAdapter::CalculateMeasurementDerivatives(
      GmatBase* obj, Integer id)
{
   #ifdef DEBUG_DERIVATIVE_CALCULATION
      Integer parmId = GetParmIdFromEstID(id, obj);
      MessageInterface::ShowMessage("Enter DSNPNRangeAdapter::CalculateMeasurementDerivatives(%s, %d) called; parm ID is %d; Epoch %.12lf\n", obj->GetName().c_str(), id, parmId, cMeasurement.epoch);
   #endif
   ...
   if (paramName == "Bias")
   {
      ...
      // For example, if solve-for object's name is 'P0.P1_ErrorModelName'.                                         // made changes by TUAN NGUYEN
      // That means: solver-for variable Bias' is related to the signal path ('P0', 'P1', ... 'Pn').                // made changes by TUAN NGUYEN
      // Ideally, the name should be 'P0.P1_P2_...._Pn_ErrorModelName'. It should be contain the name               // made changes by TUAN NGUYEN
      // of all participants in the signal path. It is a long name. Therefore, we need to make it                   // made changes by TUAN NGUYEN
      // shorter by take the names of first 2 participants. As a result, the name is 'P0.P1_ErrorModelName'.        // made changes by TUAN NGUYEN

      // We need to take derivative the measurement value of that path w.r.t. 'Bias' parameter in                   // made changes by TUAN NGUYEN
      // 'ErrorModelName' object. Before taking derivative, it needs to check participant[0] to be 'P0' and         // made changes by TUAN NGUYEN
      // participant[1] = 'P1' and so on, then calculate derivative. If its is not matched set 0 to derivative      // made changes by TUAN NGUYEN
      // because the measurement value of that signal path is independent of the solver-for bias.                   // made changes by TUAN NGUYEN

      StringArray *nameList = participantLists[0];
      for (Integer indx = 0; indx < nameList->size(); indx++)
      {
         size_t pos = name.find(nameList->at(indx));                                                                // made changes by TUAN NGUYEN
         //if (name.rfind(nameList->at(indx)) != std::string::npos)                                                 // made changes by TUAN NGUYEN
         if (pos != std::string::npos)
         {
            //std::string fullCheck = name.substr(name.rfind(nameList->at(indx)) + nameList->at(indx).length(),1);  // made changes by TUAN NGUYEN
            std::string fullCheck = name.substr(pos + nameList->at(indx).length(), 1);                              // made changes by TUAN NGUYEN
            if (fullCheck == "_" || fullCheck == ".")
            {
               // name.erase(name.rfind(nameList->at(indx)), nameList->at(indx).length());                          // made changes by TUAN NGUYEN
               name.erase(pos, nameList->at(indx).length() + 1);     // It needs to remove "_" or "." as well       // made changes by TUAN NGUYEN
               checker++;
            }
         }
      }

      if (((ErrorModel*)obj)->GetStringParameter("Type") == "DSN_PNRange" && checker > 1)
   ...
   {
      if (((ErrorModel*)obj)->GetStringParameter("Type") == "DSN_PNRange")
      {
   ...
   #ifdef DEBUG_DERIVATIVE_CALCULATION
      MessageInterface::ShowMessage("Exit DSNPNRangeAdapter::CalculateMeasurementDerivatives():\n");
   #endif
 ...
 */
//------------------------------------------------------------------------------
bool DSNPNRangeAdapter::WriteMeasurements()
{
   bool retval = false;
 ...
 */
//------------------------------------------------------------------------------
bool DSNPNRangeAdapter::WriteMeasurement(Integer id)
{
   bool retval = false;
 ...
 */
//------------------------------------------------------------------------------
Integer DSNPNRangeAdapter::HasParameterCovariances(Integer parameterId)
{
   Integer retval = 0;
 ...
 */
//------------------------------------------------------------------------------
Integer DSNPNRangeAdapter::GetEventCount()
{
   Integer retval = 0;
 ...
 */
//------------------------------------------------------------------------------
void DSNPNRangeAdapter::SetCorrection(const std::string& correctionName,
      const std::string& correctionType)
{
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::GetFrequencyFactor(Real frequency)
{
   Real factor;
 ...
 */
//------------------------------------------------------------------------------
Real DSNPNRangeAdapter::IntegralRampedFrequency(GmatTime t1, Real delta_t, Integer& err)
{
