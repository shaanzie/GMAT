...
Real UtcDate::ToA1Mjd() const
{
   ...
   if (second >= 60)
   {
      ...
      a1Mjd = a1Mjd - (1.0 / 86400.0);
      //Call is just to check if is Leap Second. If it is, return a1Mjd - 1 second. If not, error out.
      TimeSystemConverter::Instance()->ConvertToTaiMjd(TimeSystemConverter::A1MJD, a1Mjd, JD_NOV_17_1858, &isInLeapSecond);
      if (!isInLeapSecond)
      {
         a1Mjd = a1Mjd + (1.0 / 86400.0);
         //"2000-01-01T11:59:28.000"
         std::string dateStr = std::to_string(year) + "-" + std::to_string(month) + "-" + std::to_string(day) + "T" + std::to_string(hour) + ":" + std::to_string(minute) + ":" + std::to_string(second);
         throw UtilityException("UTC date " + dateStr + " has more than 60 seconds but is not on any known leap second.\n");
      }
   }
