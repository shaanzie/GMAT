...
//------------------------------------------------------------------------------
/**
 * Retrieves the (Integer) solver status
 *
...
Integer GmatGlobal::GetSolverStatus(const std::string &forSolver)
{
   Integer retval = Gmat::UNKNOWN_STATUS;       // Either solver isn't in config, or hasn't been used
...
std::string GmatGlobal::GetSolverStatusString(const std::string &forSolver)
{
   ...
   case Gmat::IN_TOLERANCE:
      status = "PhysicallyConverged";
      break;
   ...
   case Gmat::EXCEEDED_LINE_SEARCH:
      status = "ExceededLineSearches";
      break;

   case Gmat::UPHILL_SEARCH:
      status = "MinimizingUphill";
      break;

   case Gmat::INFEASIBLE:
